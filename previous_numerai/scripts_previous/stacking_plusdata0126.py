...
#('scaler', MinMaxScaler(copy=True, feature_range=(0, 1)))])
    ...
    clfs = [RandomForestClassifier(n_estimators=400, n_jobs=-1, criterion='gini'),
            ...
            LR(),
            xgb.XGBClassifier(colsample_bytree=1., learning_rate=0.01,
            max_depth=5, min_child_weight=5,n_estimators=200, nthread=-1, objective='binary:logistic', seed=0,silent=True, subsample=1.,gamma=3),
            xgb.XGBClassifier(colsample_bytree=1., learning_rate=0.01,
            ...
            #Classifier(layers=[Layer("Rectifier", units=5, dropout=0.5),Layer("Softmax")],learning_rate=0.01,n_iter=15,learning_rule='sgd',verbose=True) 
            ]
