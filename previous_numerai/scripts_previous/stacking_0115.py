...
#
#   2015/1/15
#   final_linear_stage2_0115 -> 0.5504
#
...
def run(X=None, y=None, X_submission=None, y_submission_val=None,pred_type='prediction',train_file='',test_file='',output_file='',stacked_level='stage1',creating_next_data=False,clfs=[]):#or validation
    ...
    global test_num, validation_flag, auc_stage
...
if __name__ == '__main__':
    ...
    test_file = 'numerai_datasets/numerai_tournament_data.csv'  #  test data新しいデータはココ！
    ...
    clfs = [RandomForestClassifier(n_estimators=600, n_jobs=-1, criterion='gini'),
            RandomForestClassifier(n_estimators=800, n_jobs=-1, criterion='entropy'),
            ExtraTreesClassifier(n_estimators=500, n_jobs=-1, criterion='gini'),
            ExtraTreesClassifier(n_estimators=700, n_jobs=-1, criterion='entropy'),
            xgb.XGBClassifier(colsample_bytree=0.9, learning_rate=0.01,
            max_depth=7, min_child_weight=1,n_estimators=800, nthread=-1, objective='binary:logistic', seed=0,silent=True, subsample=0.8),
            xgb.XGBClassifier(colsample_bytree=1, learning_rate=0.05,
            max_depth=8, min_child_weight=1,n_estimators=500, nthread=-1, objective='binary:logistic', seed=71,silent=True, subsample=0.8),
            xgb.XGBClassifier(colsample_bytree=1, learning_rate=0.1,
            max_depth=8, min_child_weight=1,n_estimators=350, nthread=-1, objective='binary:logistic', seed=401,silent=True, subsample=0.65),
            xgb.XGBClassifier(colsample_bytree=1, learning_rate=0.01,
            max_depth=6, min_child_weight=1,n_estimators=500, nthread=-1, objective='binary:logistic', seed=40,silent=True, subsample=0.9),
    ...
    final_prediction.to_csv( 'prediction/final_linear_stage2_0115.csv', columns = ( 't_id', 'probability' ), index = None )
